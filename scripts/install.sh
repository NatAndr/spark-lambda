...
install_scala() {
    ...
    wget -q https://downloads.lightbend.com/scala/2.12.3/scala-2.12.3.deb
...
install_spark() {
    wget -q https://archive.apache.org/dist/spark/spark-${SPARK_VERSION}/spark-${SPARK_VERSION}-bin-hadoop${HADOOP_VERSION_FOR_SPARK}.tgz
...
copy_jars() {
  ...
  wget -q https://repo1.maven.org/maven2/io/delta/delta-core_2.12/1.0.0/delta-core_2.12-1.0.0.jar
  cd /
}
